...
export class MessageModel {
    ...
    static async findById(id) {
        ...
        const message = await pool.execute(async (conn) => {
            return conn.get('SELECT * FROM messages WHERE id = ?', [id]);
        });
        if (!message) {
    ...
    static async findRecent(limit = 50) {
        ...
        return pool.execute(async (conn) => {
            return conn.all('SELECT * FROM messages ORDER BY created_at DESC LIMIT ?', [limit]);
        });
    }
    /**
    ...
    static async findByUserId(userId, limit = 50) {
        const pool = getPool();
        return pool.execute(async (conn) => {
            return conn.all('SELECT * FROM messages WHERE user_id = ? ORDER BY created_at DESC LIMIT ?', [userId, limit]);
        });
    }
    /**
     * Delete message

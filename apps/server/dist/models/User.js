...
export class UserModel {
    ...
    static async create(username, password, email) {
        ...
        const result = await pool.execute(async (conn) => {
            return conn.run('INSERT INTO users (username, password, email) VALUES (?, ?, ?)', [username, hashedPassword, email]);
        });
    ...
    static async findByUsername(username) {
        ...
        return pool.execute(async (conn) => {
            return conn.get('SELECT * FROM users WHERE username = ?', [username]);
        });
    }
    /**
    ...
    static async findAll() {
        const pool = getPool();
        return pool.execute(async (conn) => {
            return conn.all('SELECT id, username, email, created_at, updated_at FROM users');
        });
    }
    /**
     * Update user
